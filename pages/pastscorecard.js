...
const Scorecard = () => {
  ...
  const { matchId , edition } = router.query;
  console.log(matchId, edition);
  ...
  useEffect(() => {
    if (matchId) {
      const matchesRef = collection(db, "pastYearMatches");
      const matchesQuery = query(matchesRef, where("edition", "==", edition));
      ...
      const unsubscribe = onSnapshot(matchesQuery, (querySnapshot) => {
        if (!querySnapshot.empty) {
          querySnapshot.forEach((doc) => {
            ...
            // console.log(data.id);
            if (data.id == matchId) {
                // console.log(data); // Only process if matchId matches
              setMatchData(data);
              ...
              setmale_color(data.category === "male");
            }
          });
        } else {
          console.log("No match data available for the current edition");
        }
      });
